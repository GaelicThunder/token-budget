---
name: token-budget
description: "Hard token-saving mode for the whole session: minimal output, web research always delegated to a cheap Sonnet subagent, other subagents avoided, optional per-window caps on the 5-hour / weekly usage limits (no cap → default: wind down at 5% of the 5-hour window, stop at 10%). Levels: low, medium, ultra (ultra = max savings). Use when the user invokes /token-budget, mentions token budget, saving tokens, risparmiare token, or wants to limit Claude usage. Args: [low|medium|ultra] [5h=<n>] [weekly=<n>] — caps labeled by window, one is enough."
...
2. No caps given → NO questions: the **default** applies — wind down at 5% of the **5-hour window** used, hard stop at 10% of it. The weekly window is not monitored unless explicitly capped. An explicit cap overrides the default.
...
5. Confirm in ONE line naming windows explicitly, e.g.: `Token budget ON — ultra, 5h: max 20%, weekly: uncapped.` or `Token budget ON — medium, default: stop at 10% of the 5h window.` Never state a % without naming its window — here or anywhere later in the session.
6. Rules hold for the entire session, until the user says "stop token budget".

## All levels

- **Code/file subagents: avoid.** If truly unavoidable, ONE at a time, `model: sonnet`. Never opus/inherit for delegated work. No parallel fan-outs, no Workflow tool.
- **Web research: ALWAYS delegated.** Spawn ONE cheap subagent (`model: sonnet`; haiku for trivial lookups) with a tight prompt; it searches/reads the pages and returns a short digest (facts + URLs, no raw dumps). Raw web content must never enter the main context: cheap-model tokens weigh far less on the 5h/weekly limits, and the main context stays small — it gets resent on every single call. If subagents are unavailable, fetch inline and extract the minimum.
...
- **Web search: NEVER blocked, at any level** — most tasks need it. Keep it lean: targeted queries, no re-fetching the same page.

## Levels

### low — light savings
- Concise but complete answers; explanations only when asked.
- Subagents: the web delegate + at most 1 more Sonnet, only if clearly cheaper than inline.
...
- No subagents beyond the web delegate, unless blocked without one (then 1× Sonnet, tight prompt, minimal output requested).
- Read ≤200 lines per call. Prefer asking the user for exact paths over broad searching.
- Commit messages: subject line only.

### ultra — max savings
- Subagents: ONLY the web-research delegate (one at a time, digest ≤10 lines, no fan-out). Everything else inline.
...
- Thresholds, per window:
  ...
  - **Default** (no explicit caps): wind down at 5% of the **5-hour window** used, hard stop at 10% of it. Weekly: monitored only if explicitly capped.
